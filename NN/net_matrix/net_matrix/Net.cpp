...
#include "Net.h"
Net::Net(int input_size, vector<unsigned int> hidden_size_list, int output_size, string mode, bool dropout_train)
{
    dropout = dropout_train;
    Layer layer(mode);
    if(hidden_size_list.size()==0){
        Layer layer_1("softmax"); //output layer
        layer_1.weight_ = random_weight(input_size, output_size);
    ...
    for(int i=0; i<hidden_size_list.size()-1; i++)
    {
        Layer layer(mode);
...
void Net::predict(vector<vector<double> > input, bool train, vector<vector<double> > target)
{
    ...
    for(int i=0; i<layers_.size()-1; i++)
    {
        ...
        {
            layers_.back().output_layer_forward(layers_[layers_.size()-2].x_act_, target);
        }
        else
        {
        ...
        current_layer.hidden_layer_forward(last_layer.x_act_drop_);
...
{
    predict(input, dropout, target);
    net_loss_ = layers_.back().loss_;
}
...
void Net::gradient(vector<vector<double> > input, vector<vector<double> > target)
{
    ...
    {
        layers_[i].hidden_layer_backward(layers_[i+1].dx_drop_);
        grads_weight.push_back(layers_[i].dweight_);
        grads_bias.push_back(layers_[i].dbias_);
        layers_[i].dx_drop_ = drop_out_backward(layers_[i].x_act_drop_, dropout, layers_[i].dx_);
    }

}

vector<vector<double> > Net::random_weight(int currentlayer_size, int nextlayer_size)
{
    ...
    for(int i=0; i<currentlayer_size;i++)
    {
        for(int j=0; j<nextlayer_size;j++)
        {
            temp[i][j] = dist(mt);
        }
    }
...
vector<double> Net::random_bias(int currentlayer_size)
{
...
void Net::weight_update()
{
    ...
    {
        for(int j=0; j<layers_[i].weight_.size();j++)
        {
            for(int k=0; k<layers_[i].weight_[0].size();k++)
            {
...
void Net::bias_update()
{
    for(int i=0; i<layers_.size();i++)
    {
        for(int j=0; j<layers_[i].bias_.size();j++)
        {
...
void Net::show()
{
        ...
        for(int i=0; i<layers_.size();i++)
        {
            for(int j=0; j<layers_[i].x_act_drop_.size();j++)
            {
                cout << "[";
                for(int k=0; k< layers_[i].x_act_drop_[0].size();k++)
                {
        ...
        cout << "dweight" << endl;
        for(int i=0; i<grads_weight.size();i++)
        {
            for(int j=0; j<grads_weight[i].size();j++)
            {
                cout<< j << endl;
                cout << "[";
                for(int k=0; k< grads_weight[i][j].size();k++)
                {
        ...
        cout << "dbias" << endl;
        for(int i=0; i<grads_bias.size();i++)
        {
            cout << "[";
            for(int j=0; j<grads_bias[i].size();j++)
            {
...
double Net::accuracy()
{
    ...
    int max;
    int max_1;
    for(int i=0; i<layers_.back().y_.size();i++)
    {
        for(int j=0; j<layers_.back().y_[0].size();j++)
        {
            max = 0;
...
//            cout << max;
        ...
        for(int k=0; k<layers_.back().t_[0].size();k++)
        {
            max_1 = 0;
